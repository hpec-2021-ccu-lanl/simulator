...
Options:
    --config=config      configuration file
                         [default: /home/sim/basefiles/configs/1_simulation.config]
...
experimentsNum=len(experiments)
experimentsCount=1
for exp in experiments:
    ...
    for job in jobs:
        for number in range(start,end+1,1):
            print("Experiment "+str(experimentsCount)+"/"+str(experimentsNum),flush=True)
            print("Job " + str(jobsCount)+"/"+str(jobsNum),flush=True)
            print("Run " + str(number)+"/"+str(end),flush=True)
            run = "Run_" + str(number)
